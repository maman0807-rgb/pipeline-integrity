...
function PriorityBadge({ p }) {
  const map = { P1: 'text-red-400', P2: 'text-yellow-400', P3: 'text-green-400' }
...
export default function Flowline() {
  ...
  async function load() {
    setLoading(true)
    const { data } = await supabase.from('pipelines').select('*').order('dari_sumur')
    setRows(data || [])
    setLoading(false)
  }
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-2xl p-6 w-full max-w-2xl my-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                <div key={key} className={key === 'nama_flowline' || key === 'catatan' ? 'col-span-2' : ''}>
            ...
            <div className="flex justify-end gap-3 mt-5">
              <button onClick={closeModal} className="px-4 py-2 text-sm text-slate-400 hover:text-white">Batal</button>
